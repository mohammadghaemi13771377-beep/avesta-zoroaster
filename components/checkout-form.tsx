...
export function CheckoutForm({ products, initialItems }: { products: ShopProduct[]; initialItems: CheckoutCartItem[] }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="checkout-form-grid grid gap-6 lg:grid-cols-[1fr_380px]">
      <section className="checkout-form-panel lux-frame p-6">
        ...
        <div className="mt-6 grid gap-4">
          ...
          <label className="checkout-field grid gap-2">
            ...
            <select
              ...
              className="checkout-input h-12 rounded-2xl border border-gold/20 bg-royal px-4 text-warm outline-none focus:border-gold"
            >
          ...
          <button
            ...
            className="checkout-submit-button inline-flex w-fit items-center gap-2 rounded-full bg-gold px-6 py-4 font-black text-night transition hover:bg-gold-light disabled:cursor-not-allowed disabled:opacity-60"
          >
          ...
          {paymentUrl ? (
            <Link
              href={paymentUrl}
              className="checkout-payment-link inline-flex w-fit items-center gap-2 rounded-full border border-gold/30 px-6 py-4 font-black text-gold-light transition hover:bg-gold/10"
            >
              ادامه در پرداخت آزمایشی
              <CreditCard size={18} />
            </Link>
          ) : null}
          <p className="checkout-status text-sm leading-7 text-muted">{status}</p>
        </div>
      </section>

      <aside className="checkout-summary-panel lux-frame h-fit p-5">
        <div className="flex items-center gap-2 text-gold-light">
        ...
        <div className="mt-5 grid gap-3">
          {cartItems.length ? (
            cartItems.map((item) => (
              <div key={item.slug} className="checkout-summary-item rounded-2xl border border-gold/10 bg-night/55 p-4">
            ...
            <div className="checkout-summary-item rounded-2xl border border-gold/10 bg-night/55 p-4 text-sm leading-7 text-muted">
              محصولی در query checkout نیست. از فروشگاه یک محصول اضافه کن.
            </div>
          )}
        </div>
        <div className="checkout-total-box mt-5 rounded-3xl border border-gold/20 bg-gold/10 p-5">
  ...
  return (
    <label className="checkout-field grid gap-2">
      ...
      <input
        ...
        className={`checkout-input h-12 rounded-2xl border border-gold/20 bg-royal px-4 text-warm outline-none placeholder:text-muted focus:border-gold ${
          ltr ? "text-left" : ""
        }`}
      />
    </label>
  ...
  return (
    <label className="checkout-field grid gap-2">
      ...
      <textarea
        ...
        onChange={(event) => onChange(event.target.value)}
        rows={rows}
        className="checkout-input rounded-2xl border border-gold/20 bg-royal p-4 text-warm outline-none placeholder:text-muted focus:border-gold"
      />
    </label>
  );
}
