        ...
        <div className="fixed inset-0 z-[90] bg-black/75 backdrop-blur-xl" role="dialog" aria-modal="true" aria-label="فهرست ناوبری سایت">
          ...
          <aside className="absolute inset-y-0 right-0 flex w-[min(88vw,390px)] flex-col border-l border-gold/20 bg-[#071521] shadow-2xl shadow-black/70">
            ...
            <div className="flex-1 overflow-y-auto p-5">
              <Link href="/search" onClick={() => setOpen(false)} className="flex items-center justify-between rounded-2xl border border-gold/20 bg-gold/10 px-4 py-3 text-sm font-black text-gold-light">
              ...
              <nav className="mt-6 grid gap-1">
                {navItems.map((item) => (
                  <Link key={item.href} href={item.href} onClick={() => setOpen(false)} className="flex items-center justify-between rounded-xl px-3 py-3 text-base font-black text-warm transition hover:bg-gold/10 hover:text-gold-light">
                ...
                {avestaOpen ? (
                  <div className="mt-2 grid gap-1 rounded-2xl border border-gold/12 bg-night/35 p-2">
                    {avestaSections.map((section) => (
                      <Link key={section.slug} href={section.href} onClick={() => setOpen(false)} className="rounded-xl px-3 py-2.5 text-sm font-bold text-warm/85 transition hover:bg-gold/10 hover:text-gold-light">
                        {section.title}
                      </Link>
                    ))}
                  </div>
                ...
                {exploreOpen ? (
                  <div className="mt-2 grid gap-1 rounded-2xl border border-gold/12 bg-night/35 p-2">
                    {exploreNavItems.map((item) => (
                      <Link key={item.href} href={item.href} onClick={() => setOpen(false)} className="rounded-xl px-3 py-3 transition hover:bg-gold/10">
